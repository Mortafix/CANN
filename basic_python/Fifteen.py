...
def print_number(n,dim):
	...
	n_len = len(str(dim*dim))
	if n != 0:
		for i in range(0,n_len-len(n_s)):
			s += " "
		s += n_s
	else:
...
def create_random_board(dim):
	...
	return [board[i:i+dim] for i in range(0,len(board),dim)]
...
def inversions_number(board,dim):
	...
	for j in range(0,len(board_stretch)):
		for k in range(j+1,len(board_stretch)):
